...
class TestInlineMarkdown(unittest.TestCase):
    ...
    def test_extract_markdown_links(self):
        matches = extract_markdown_links(
            "This is text with a [link](https://generic.link) and [another link](https://test.link)"
        )
        self.assertListEqual(
            [
                ("link", "https://generic.link"),
                ("another link", "https://test.link"),
            ],
            matches,
        )
    ...
    def test_split_image(self):
        node = TextNode(
            "This is text with an ![image](https://www.example.com/image.png) and another ![second image](https://www.example.com/image1.png)",
            TextType.TEXT,
        )
        new_nodes = split_nodes_image([node])
        self.assertListEqual(
            [
                ...
                TextNode(
                    "second image", TextType.IMAGE, "https://www.example.com/image1.png"
                ),
            ],
            new_nodes,
        )
